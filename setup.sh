...
# Setup script for the UiPath Action App template
...
NEW_PROJECT_NAME="$1"
echo "Setting up UiPath Action App project: $NEW_PROJECT_NAME"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS sed syntax
    sed -i '' "s/\"name\": \"uipath-action-app\"/\"name\": \"$NEW_PROJECT_NAME\"/g" wrangler.jsonc
else
    # Linux sed syntax
    sed -i "s/\"name\": \"uipath-action-app\"/\"name\": \"$NEW_PROJECT_NAME\"/g" wrangler.jsonc
fi
...
echo "[OK] Updated project name to: $NEW_PROJECT_NAME"
echo ""
echo "Next steps:"
echo "1. npm install (or bun install)"
echo "2. npm run dev (to start development server)"
echo "3. Define your action schema in src/types/action-schema.ts"
echo "4. Customize your ActionPage in src/pages/ActionPage.tsx"
echo "5. npm run deploy (when ready to deploy)"
